# VSCode
**VSCode** is used interchangeably with **VSCodium** and **Code - OSS**. The
later two are VSCode without telemetry. These are exclusively recommended over
VSCode.

=== "CachyOS"
    ``` bash
    pacman -S code  # Optimized Code - OSS (preferred).
    paru -S code-marketplace  # Code - OSS auto patch with MS Marketplace.
    pacman -S vscodium  # VSCodium.
    ```
