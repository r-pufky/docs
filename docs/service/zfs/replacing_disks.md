...
!!! tip
    When issuing ZFS commands against devices in the pool, you **must** use
    those ID's.
...
Determine the bad disk based on the status returned. This disk ID may be the
WWN or the full path to the disk. If it is the WWN, determine the block device.
Identify the drive serial number for easy hardware swapping when off.

### Identify block devices to minimize downtime
``` bash
zpool status {POOL}
...
Immediately after this command ZFS will scan the pool.
``` bash
zpool status
>   pool: tank
>  state: DEGRADED
...
After the initial scan, re-silvering should kickoff.
``` bash
zpool status
>   pool: tank
>  state: DEGRADED
...
In-place replacement with no disk device ID change.
``` bash
zpool scrub {POOL}
zpool status {POOL}
lsblk
...
Replacement with new disk device ID.
``` bash
{ADD NEW DISK}
zpool scrub {POOL}
zpool status {POOL}
...
Upgrade process for each disk in pool.
``` bash
zpool scrub {POOL}
zpool status {POOL}
lsblk
...
Repeat for each disk. If constrained by hardware the old disk can be off-lined
the new disk physically changed, and then replaced.
``` bash
...
ZFS manages all disk hardware and - depending on how the pool was created, when
disks were added, removed, or replaced - you may end up with some devices
showing partitions versus raw disks. This is **OK**. The zpool listing is just
the ID used to represent the disk.
...
Show the underlying hardware configuration for a ZFS pool.
``` bash
zdb
>   ...
> children[2]:
