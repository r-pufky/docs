...
Copy with robocopy (powershell as admin)
``` powershell
...
!!! example "⌘ ➔ winscp ➔ Tools ➔ Export/Backup Configuration"

Dump winscp settings to reg file.
``` powershell
# Run as administrator.
...
Dump mumble client settings to reg file.
``` powershell
# Run as administrator.
