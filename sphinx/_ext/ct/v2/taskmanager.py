...
class TaskManager(ct.AbstractConfigTable):
  ...
  Directives:
    :path:        String GUI navigation. Required.
  ...
  conf.py options:
    ct_taskmanager_separator: Unicode separator to use for path. This uses the
    ...
    ct_taskmanager_separator_replace: String separator to replace with
  ...
  Examples:
    .. taskmanager:: Disable windows defender notification icon manager
      ...
      Dropdown opened by default, using label.
    ...
    .. taskmanager:: Disable windows defender notification icon manager
      ...
      Dropdown opened by default, using commas as delim. Generic header.
  ...
  def _add_dropdown_header(self):
    if 'generic' in self.options:
      self._rst.append(".. dropdown:: Task Manager", self.c)
...
def setup(app):
  app.add_config_value('ct_taskmanager_separator', config.DEFAULT_SEPARATOR, '')
  app.add_config_value('ct_taskmanager_separator_replace', config.DEFAULT_REPLACE, '')
  app.add_directive('taskmanager', TaskManager)
